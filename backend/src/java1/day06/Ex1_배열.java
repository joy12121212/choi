		...
		// 3. String 클래스 배열 
			...
			arr3[1] = null;		System.out.println( Arrays.toString( arr3 ) );
				...
				for( int i = 1 ; i < arr3.length ; i++ ) { 
					...
					arr3[i]=arr3[i+1]; // i가 1이라면 i+1 = 2 // 2번 인덱스에 저장된 값을 i번째 인덱스에 저장한다 
					// 삭제할 데이터(인덱스번호)를 알고 거기서 부터 for 문 실행
					// 해당 데이터에 null 넣고 for문 돌리는게 아니라 그냥 다음 인덱스에 저장된 데이터를 땡겨온다
					// 마지막 인덱스(arr3.length-1)에 도달하면 그곳에 저장할게 없으니 null 입력 후 브레이크
				}
				System.out.println( Arrays.toString( arr3 ) );
