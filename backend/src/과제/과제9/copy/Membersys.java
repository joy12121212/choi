...
public class Membersys {
	public static void main(String[] args) {
		...
		// memberList(힙 영역 주소가 저장 되어있다)	: 스택
		// = newMember[100];					: 힙 
		...
		while (true) {
			...
			// scanner.nextInt();	: 힙 scanner 클래스의 nestInt(); 함수 호출
			// int sc1				: 스택
			...
			if (sc1 == 1) {
				...
				// member.id  ->  member에 Member() 클래스 주소가 저장됨 -> member == Member() 
				// member.id -> Member()클래스의 필드 id 를 찾아서 실행 한다
