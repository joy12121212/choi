...
//				System.out.println(제품정보);
				...
				//재고관리 업데이트
				재고관리[ch-1] = 재고+","+바구니+","+ 가격+","+이름;  //재고관리의 n번 인덱스에 재고, 바구니, 가격 재고 현행화
...
//					System.out.println(Arrays.toString(재고관리)); // 재고관리 배열이 모든 정보 호출/출력
					...
					for (int i = 0; i < 재고관리.length; i++) {//0부터 재고관리의 길이 까지 for 문 실행
						...
						int 바구니 = Integer.parseInt(재고관리[i].split(",")[1]); // 재고관리 배열의  i 번째 인덱스에서 , 로 구분 지었을때 1번 인덱스의 값을 stock에 대입 한다
						...
						if (바구니 > 0) {//만약에 stock 이 0보다 크다면 아래 명령어 실행
							System.out.printf("%10s %10s %10s \n" , name , 바구니 , 바구니*price);
